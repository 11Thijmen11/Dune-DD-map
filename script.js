document.addEventListener('DOMContentLoaded', function() {
    ...
    function saveMapState() {
        ...
        document.querySelectorAll('.cell').forEach(cell => {
            ...
            if (cell === selectedCell) {
                const subCellsState = {};
                popupSubGrid.querySelectorAll('.sub-cell').forEach(subCell => {
                    if (subCell.dataset.type) {
                        subCellsState[subCell.dataset.index] = subCell.dataset.type;
                    }
                });
                if (Object.keys(subCellsState).length > 0) {
                    mapState[coordinates] = subCellsState;
                }
            } else {
                const savedState = getSavedSubCellsState(coordinates);
                if (savedState) {
                    mapState[coordinates] = savedState;
                }
            }
        });
    ...
    function loadMapState() {
        ...
        if (savedState) {
            const mapState = JSON.parse(savedState);
            Object.entries(mapState).forEach(([coordinates, subCellsState]) => {
                const cell = document.querySelector(`[data-coordinates="${coordinates}"]`);
                if (cell) {
                    let itemsAdded = 0;
                    let firstType = null;
                    Object.entries(subCellsState).forEach(([index, type]) => {
                        if (type) {
                            itemsAdded++;
                            if (itemsAdded === 1) firstType = type;
                        }
                    });
                    if (itemsAdded > 0) {
                        cell.classList.add('has-item');
                        if (itemsAdded === 1) {
                            createOrUpdateIcon(cell, firstType, false);
                        } else {
                            const types = Object.values(subCellsState).filter(Boolean).slice(0, 3);
                            createOrUpdateIcon(cell, types, true);
                        }
                    }
                }
            });
